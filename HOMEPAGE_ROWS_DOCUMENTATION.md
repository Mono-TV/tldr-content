...
### 6. Top Action Movies by Language

**Overview:**
Top Action Movies rows showcase the best action films across different languages, using the same rating and vote thresholds as the Top Rated Movies rows. Each row filters by the "Action" genre while maintaining language-specific quality standards.
...
**General Action (61 total):**
- Dhurandhar (8.6, Hindi)
...
**Use Case:**
- Highlight best action films in each language
- Leverage popularity of action genre across all markets
- Maintain quality standards with language-appropriate thresholds
- Easy discovery of high-rated action content by language preference
...
| Language | ISO Code | Rating | Votes | Years | Results | Implementation |
|----------|----------|--------|-------|-------|---------|----------------|
| General (All) | - | 7.5+ | 50,000 | 10 | 66 | Cross-language comedy movies |
| English | `en` | 7.5+ | 50,000 | 10 | 56 | Hollywood comedy films |
| Hindi | `hi` | 7.5+ | 10,000 | 10 | 25 | Bollywood comedy cinema |
...
- IMDb rating: Language-specific (7.5/7.0/6.5 based on content availability)
- Vote count: Language-specific threshold (adjusted for genre-specific availability)
- Content type: movie
- Year from: Language-specific (10/15/20/25 years based on content volume)
- Sorted by: IMDb rating (descending) → Vote count (descending) → Release date (descending)
...
- **General**: 7.5 rating, 50k votes - quality cross-language comedy (66 results)
- **English**: 7.5 rating, 50k votes - quality Hollywood comedy (56 results)
- **Hindi**: 7.5 rating, 10k votes - quality Bollywood comedy (25 results)
...
**Browse Links:**
- `/browse?genre=Comedy&min_rating=7.5&sort=rating` (General)
- `/browse?language=English&genre=Comedy&min_rating=7.5&sort=rating` (English)
- `/browse?language=Hindi&genre=Comedy&min_rating=7.5&sort=rating` (Hindi)
...
| Language | ISO Code | Rating | Votes | Years | Results | Implementation |
|----------|----------|--------|-------|-------|---------|----------------|
| General (All) | - | 8.0+ | 50,000 | 10 | 56 | Cross-language drama movies |
| English | `en` | 8.0+ | 50,000 | 10 | 28 | Hollywood drama films |
| Hindi | `hi` | 7.5+ | 50,000 | 10 | 16 | Bollywood drama cinema |
...
- **General**: 8.0 rating, 50k votes - highest quality cross-language drama (56 results)
- **English**: 8.0 rating, 50k votes - maintain high standards for Hollywood (28 results)
- **Hindi**: 7.5 rating, 50k votes - quality Bollywood drama (16 results)
...
- `/browse?language=Hindi&genre=Drama&min_rating=7.5&sort=rating` (Hindi)
...
| Language | ISO Code | Rating | Votes | Years | Results | Implementation |
|----------|----------|--------|-------|-------|---------|----------------|
| General (All) | - | 8.0+ | 50,000 | 10 | 18 | Cross-language thriller movies |
| English | `en` | 7.5+ | 50,000 | 10 | 27 | Hollywood thriller films |
...
- **General**: 8.0 rating, 50k votes - highest quality cross-language thriller (18 results)
- **English**: 7.5 rating, 50k votes - quality Hollywood thriller (27 results)
...
- `/browse?language=English&genre=Thriller&min_rating=7.5&sort=rating` (English)
...
curl -s "https://content-api-401132033262.asia-south1.run.app/api/content?min_rating=8&min_votes=50000&type=movie&genre=Action&year_from=2016&sort=rating&order=desc&limit=15" \
  | jq '.items[0:5] | .[] | {title, imdb_rating, original_language, genres: [.genres[].name]}'
```
...
### 4b. Top Tamil Action Movies

```bash
curl -s "https://content-api-401132033262.asia-south1.run.app/api/content?min_rating=8&min_votes=15000&type=movie&genre=Action&original_language=ta&year_from=2016&sort=rating&order=desc&limit=15" \
...
| Tamil | `original_language=ta, min_rating=8, min_votes=15000, year_from=2016` | `ta` |
| Telugu | `original_language=te, min_rating=8, min_votes=5000, year_from=2016` | `te` |
| Malayalam | `original_language=ml, min_rating=8, min_votes=5000, year_from=2016` | `ml` |
| Kannada | `original_language=kn, min_rating=8, min_votes=5000, year_from=2011` | `kn` (15 yrs) |
| Bengali | `original_language=bn, min_rating=7.5, min_votes=2000, year_from=2011` | `bn` (15 yrs) |
